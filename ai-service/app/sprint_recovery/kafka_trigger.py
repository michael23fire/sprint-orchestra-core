...
class SprintRecoveryKafkaTrigger:
    ...
    def register_waiting(self, sprint_id: int, thread_id: str) -> None:
    ...
    async def _handle(self, raw: bytes) -> None:
        ...
        logger.info(
            ...
            extra={"issue_key": issue_key, "space_id": space_id, "waiting_sprints": list(self._waiting_by_sprint.keys())},
        )
        if not issue_key or space_id is None:
            return

        from app.sprint_recovery.graph import build_escalation_summary, trigger_reevaluation
        ...
        # waiting on? We don't have the issue's sprint_id in the event envelope itself, so this checks
        # every sprint currently being waited on — cheap in practice since the waiting set is small.
